...
describe('footer', () => {
  ...
  it('has the clubs name at the top', () => {
    expect(footerTextBox.children[0].props.children).to.be.equal(
      'Wheaton Community Radio Amateurs'
    );
  });
  it('has the clubs address in the middle', () => {
    expect(footerTextBox.children[1].props.children[1]).to.be.equal(
      ' • P.O. Box 1055 • Wheaton, Illinois • 60187-1055 • (630) 923-5447 • www.w9ccu.org'
    );
  });
  it('has a copywrite at the bottom', () => {
    expect(footerTextBox.children[2].props.children[0]).to.include(
      'Contents copyright ©'
    );
  });
});
