...
describe('Training - Event Section', () => {
  ...
  describe('titles', () => {
      ...
      it('has 1st header = Upcoming Technician Class Training and License Examination', () => {
        expect(titles[0].children).to.be.equal('Upcoming Technician Class Training and License Examination');
      });
  ...
  describe('no scheduled events', () => {
    ...
    it('says to check back later nothing currently schuduled', () => {
       expect(news[0].children.props.children).to.include('no classes currently schedule')
       expect(news[0].children.props.children).to.include('Please check back later')
       expect(news[1].children.props.children).to.include('no additional WCRA Exam session scheduled ')
       expect(news[1].children.props.children).to.include('Please check back later')
    })
  })
  describe('events scheduled', () => {
    ...
    let new4p;
    before(async() => {
            await store.dispatch(setEvents([...fakecalendarEvents, fakeEvent2, fakeEvent3, fakeEvent4]));
        section = mount(
                <Provider store={store}>
                  <rrd.MemoryRouter initialEntries={['/Training']}>
                    <TrainingEvents />
    ...
    it('all have a h3 header', () => {
      ...
      expect(newH1.props.children.join('')).to.be.equal('Weather Net on November 10, 2020');
      expect(newH2.props.children.join('')).to.be.equal('Tech on July 17, 2021');
      expect(newH3.props.children.join('')).to.be.equal('General on July 17, 2021');
      expect(newH4.props.children.join('')).to.be.equal('Tech on July 17, 2021');
    });
    ...
    it('all give the  time range', () => {
      expect(new1p[0].join('')).to.be.equal(
        'Time: 7:00 PM to 7:15 PM'
      );
      expect(new2p[0].join('')).to.be.equal(
        'Time: 9:00 AM to 5:00 PM'
      );
      expect(new3p[0].join('')).to.be.equal(
        'Time: 9:00 AM to 5:00 PM'
      );
      expect(new4p[0].join('')).to.be.equal(
        'Time: 9:00 AM to 5:00 PM'
      );
    });
