...
describe('Training Page', () => {
    describe('shallow Render', () => {
        ...
        it('renders a title of WCRA Radio Repeaters', () => {
            let [title] = trainingPage.find('div').map((node) => node.get(0).props).filter((prop) => prop.className === 'Title');
          ...
          it('has an h2 for Additional Resources', () => {
              expect(trainingPage.find('h2').text()).to.be.equal('Additional Resources')
          })
    })
    describe('full mount', () => {
        let trainingPage;
        before(() => {
            trainingPage = mount(<Provider store={store}><rrd.MemoryRouter initialEntries={['/training']}><Training /></rrd.MemoryRouter></Provider>)
        })
        it('renders a title of WCRA Radio Repeaters', () => {
            let [title] = trainingPage.find('div').map((node) => node.get(0).props).filter((prop) => prop.className === 'Title');
            expect(title.children).to.be.equal('Amateur Radio Classes & Testing');
          ...
          describe('h2 headers', () => {
            ...
            it('1st h2 header is for Upcoming Technician Class Training and License Examination', () => {
              expect(headers[0].children).to.be.equal('Upcoming Technician Class Training and License Examination');
            });
