...
describe('Activities - Special Event Section', () => {
  ...
  it('has a header of Special Events', () => {
    expect(section.find('h2').text()).to.be.equal('Special Events');
  });
  describe('maping of events', () => {
    ...
    it('the store arr only has 2 special events in it', () => {
      let storeArray = section
        .get(0)
        .props.store.getState()
        .calendarEvents.filter((cal) => cal.Type === 'Special Event');
    ...
    it('both have a h3 header', () => {
      ...
      expect(newH1.props.children).to.be.equal(fakeSpecialEvent.Name);
    ...
    it('both give a date and time range', () => {
      expect(new1p[0].join('')).to.be.equal(
        'January 17, 2021 from 8:00 AM to 4:00 PM'
      );
    ...
    it('both give a location in the middle p', () => {
      expect(new1p[1][1]).to.be.equal(fakeSpecialEvent.Location);
      expect(new2p[1][1]).to.be.equal(fakeEvent2.Location);
    });
    it('both give a Description if defined in the last p', () => {
      expect(new1p[2]).to.be.equal(fakeSpecialEvent.Description);
